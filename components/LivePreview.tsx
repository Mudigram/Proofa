...
export default function LivePreview({ data, type, initialTemplate = "minimalist", docId }: LivePreviewProps) {
    ...
    const renderTemplate = () => {
        const props = { data, type, isPro, currencyCode: profile?.defaultCurrency || "NGN" };
    ...
    const dynamicStyle: React.CSSProperties = customBrandColor ? {
        ...
        "--color-primary-500": customBrandColor,
    ...
    return (
        <>
            <div className="flex flex-col gap-8 w-full" style={dynamicStyle}>

                {/* Hidden clean capture target — watermark only rendered for free users */}
                <div
                    aria-hidden="true"
